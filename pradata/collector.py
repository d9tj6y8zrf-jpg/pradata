...
def coverage_counts(statuses: list[dict[str, Any]]) -> dict[str, int]:
    """Separa les fonts completes de les que almenys han respost parcialment."""
    return {
        "source_count": len(statuses),
        "successful_sources": sum(
            1 for status in statuses if status["state"] == "ok"
        ),
        "responsive_sources": sum(
            1 for status in statuses if status["state"] != "error"
        ),
    }
...
def run_collection(root: Path) -> dict[str, Any]:
    ...
    coverage = coverage_counts(statuses)
    return {
        ...
        "responsive_sources": coverage["responsive_sources"],
    }
from __future__ import annotations
